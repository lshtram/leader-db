...
Owns the per-row :class:`NormalizedObservation` build loop for
the unified-source WDI adapter. The function takes the wide-
format DataFrame returned by the local cache-only read path
(in :mod:`._cache_reader`) -- one row per ``(iso3, year)``
with one column per catalog indicator named with the
canonical ``variable_name`` -- and emits the canonical
observation records with raw locators, transform locators,
attribution text, and column-specific unit labels.

Split out of the original 802-line monolith so:

- :mod:`_cache_reader` owns the offline / cache-only read
  path (cache-file parsing, aggregate filter, long-to-wide
  pivot, cache index, ``df.attrs`` counter contract).
- :mod:`_transform` (this module) owns the observation-
  emission loop (catalog spec resolution, observation-
  family mapping, unit-label fallback, raw + transform
  locators, attribution text, observation_id string).

The split keeps each module under the 400-line convention.
For backward compatibility the symbols moved to
:mod:`._cache_reader` are re-exported here so existing
``from ._transform import load_wdi_cache_index`` /
``from ._transform import _read_cached_wdi_responses`` calls
keep working without churn.

Wide-format row emission
------------------------

The read path normalizes the staged per-(year, indicator)
long-format payloads to the cache-wide layout: one row per
...
# Re-export the cache-reader public surface so existing
# callers (``from ._transform import _read_cached_wdi_responses``
# / ``from ._transform import load_wdi_cache_index``) keep
# working without churn. The canonical definitions live in
# :mod:`._cache_reader`.
from ._cache_reader import (  # noqa: F401  (re-export)
...
# ``transform_locator``. Surfaces the cache-read transform stage
# name so downstream scoring / audit code can resolve the parser
# and locator path used for this observation.
...
# catalog ``IndicatorSpec.unit`` field is the authoritative
...
def emit_world_bank_wdi_observations(
    ...
    wide_df:
        ...
        :func:`_read_cached_wdi_responses` (one row per
    ...
    spec_by_variable_name:
        Dict mapping catalog ``variable_name`` to a
    ...
    for _, wide_row in wide_df.iterrows():
        ...
        # the index map is unavailable (for tests that do not
        # exercise pointer-resolution or for missing cache files),
        # the transform falls back
            ...
            if json_pointer is None:
                # Fallback pointer shape when index resolution is
                # unavailable. The readiness test in
                # readiness test in
