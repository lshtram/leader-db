...
def initial_metric_definitions() -> tuple[MetricDefinition, ...]:
    ...
    return (
        ...
        MetricDefinition(
            ...
            label="GDP per capita (concept)",
            description="Source-concept GDP per capita rows extracted from normalized evidence.",
            grain=VizGrain.COUNTRY_YEAR,
            value_column="value",
            unit="source_native",
            source_tables=("normalized_observations",),
            required_columns=("country_iso3", "year", "source_keys"),
            allowed_aggregations=(AggregationKind.NONE, AggregationKind.MEAN),
            allowed_transforms=(TransformKind.IDENTITY,),
            attribution_source_keys=("world_bank_wdi", "maddison_project", "pwt"),
        ),
        MetricDefinition(
            metric_id="concept.population",
            label="Population (concept)",
            description="Source-concept population rows extracted from normalized evidence.",
            grain=VizGrain.COUNTRY_YEAR,
            value_column="value",
            unit="source_native",
            source_tables=("normalized_observations",),
        ...
        MetricDefinition(
            ...
            label="GDP total (concept)",
            description="Source-concept total GDP rows extracted from normalized evidence.",
            grain=VizGrain.COUNTRY_YEAR,
            value_column="value",
            unit="source_native",
            source_tables=("normalized_observations",),
