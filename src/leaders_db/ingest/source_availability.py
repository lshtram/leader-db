...
def check_all_sources(
    ...
    *,
    ...
    raw_names = (
        {path.name for path in raw_base.iterdir() if path.is_dir()}
        if raw_base.exists()
        else set()
    )
...
def _audit_source(
    ...
    *,
    ...
    persisted = bool(
        (observation_path and observation_path.is_file()) or database_profile
    )
    identity_family = any("leader_identity" in item for item in profile["families"])
    country_matched = normalized and (profile["country_count"] > 0 or identity_family)
    concept_sources = {
    ...
    researcher_routed = slug in routed_sources and concept_mapped
    validated = all(
        (identified, vetted, raw_available, adapter_works, normalized, persisted,
         country_matched, concept_mapped, researcher_routed)
    )
