"""Per-component helpers for the political freedom scorer.
...
:mod:`leaders_db.score._political_freedom_flags`.
...
- :func:`map_normalized_to_1_10` — the half-up rounding to the
  ...
  wellbeing scorer's mapping so cross-category comparisons stay
  on one scale);
...
from ._political_freedom_rubric import (
    _GROUP_BY_VARIABLE,
    _GROUP_WEIGHTS,
    CATEGORY_KEY,
)
...
def map_normalized_to_1_10(normalized: float) -> int:
    ...
    integrity / effectiveness / economic wellbeing scorer's
...
def resolve_leader_name(bundle: CategoryEvidenceBundle) -> str:
    ...
    could not place a ruler; for the political freedom rubric
...
def filter_excluded_observations(
    ...
    :func:`score_political_freedom` as the **single** observation
