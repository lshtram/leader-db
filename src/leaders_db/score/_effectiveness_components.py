"""Per-component helpers for the effectiveness scorer.

This module is the **internal** per-component logic of the
effectiveness deterministic scorer. The facade in
:mod:`leaders_db.score.effectiveness` calls
...
:mod:`leaders_db.score._effectiveness_flags`.
...
from ._effectiveness_rubric import (
    _GROUP_BY_VARIABLE,
    _GROUP_WEIGHTS,
    CATEGORY_KEY,
)
...
def map_normalized_to_1_10(normalized: float) -> int:
    ...
    scorer's mapping and the integrity scorer's mapping.
    """
    if normalized <= 0.0:
        return 1
    if normalized >= 1.0:
...
def resolve_leader_name(bundle: CategoryEvidenceBundle) -> str:
    ...
    could not place a ruler; for the effectiveness rubric the
    leader identity is secondary to the country-year indicators,
    so we fall back to the country name in that case.
    """
...
def filter_excluded_observations(
    ...
    :func:`score_effectiveness` as the **single** observation
