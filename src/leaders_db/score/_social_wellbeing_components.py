"""Per-component helpers for the social-wellbeing scorer.
...
:func:`compute_group_components` to turn the bundle's observations
into a flat list of :class:`~leaders_db.score.results.ScoreComponent`
rows; the rationale and flag helpers live in
:mod:`leaders_db.score._social_wellbeing_flags`.

It also owns the helpers that are shared between the score path and
the insufficient-data path:

- :func:`map_normalized_to_1_10` — the half-up rounding to the
  integer 1..10 scale;
- :func:`resolve_leader_name` — the bundle leader-name → country-name
  fallback;
...
from ._social_wellbeing_rubric import (
    _GROUP_BY_VARIABLE,
    _GROUP_WEIGHTS,
    CATEGORY_KEY,
)
...
def map_normalized_to_1_10(normalized: float) -> int:
    ...
    The mapping is restated in the result's ``rationale_short`` so a
    reviewer reading the manual-review queue row sees the scale
    explicitly.
    """
    if normalized <= 0.0:
        return 1
...
def resolve_leader_name(bundle: CategoryEvidenceBundle) -> str:
    ...
    ``leader_name`` may be ``None`` when the Stage 4 resolver could
    not place a ruler; for the social-wellbeing rubric the leader
    identity is secondary to the country-year indicators, so we fall
    back to the country name in that case.
    """
...
def build_observation_ref(
    ...
    ``TemporalKind.NOT_AVAILABLE`` observations so the ref points at
    the bundle's target year — the contract requires the ref's
    ``observation_year`` to be set when the underlying observation is
    not a ``NOT_AVAILABLE`` placeholder, and the bundle builder
    already enforces that for the rows the scorer sees.
    """
    if obs.observation_year is None:
        obs_year = target_year
    else:
        obs_year = obs.observation_year
...
def bucket_observations_by_group(
    ...
    Observations whose variable is not in the group map are skipped
    (defence in depth; the bundle builder scopes rows to the variable's
    owning source so this should not happen in production).
    """
...
def compute_group_components(
    ...
    *,
    ...
    ``group_contribution`` is ``group_weight * mean(group_normalized)``
    where ``group_normalized`` is the list of per-observation
    ``normalized_value`` (skips observations with ``normalized_value``
    of ``None``). When the group has no usable observations, the
    contribution is 0.0 and the component/ref lists are empty.

    Each non-empty group emits one :class:`ScoreComponent` per
    observation with ``weight = group_weight / count_in_group`` and
    ``contribution_0_1 = normalized_value * weight``. Sum of the
    group-level contributions equals the returned
    ``group_contribution`` within float tolerance; the per-component
    breakdown is the audit trail for reviewers.
    """
