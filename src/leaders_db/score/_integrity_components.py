"""Per-component helpers for the integrity scorer.

This module is the **internal** per-component logic of the
integrity deterministic scorer. The facade in
:mod:`leaders_db.score.integrity` calls
...
:mod:`leaders_db.score._integrity_flags`.
...
- :func:`map_normalized_to_1_10` — the half-up rounding to the
  ...
  social-wellbeing scorer's mapping so cross-category
...
def resolve_leader_name(bundle: CategoryEvidenceBundle) -> str:
    ...
    could not place a ruler; for the integrity rubric the leader
    identity is secondary to the country-year indicators, so we
    fall back to the country name in that case.
    """
...
def filter_excluded_observations(
    ...
    :func:`score_integrity` as the **single** observation basis
    for the result: components, refs, contributions,
    :attr:`ScoreResult.missingness`,
    :attr:`ScoreResult.review_flags`, and
    :attr:`ScoreResult.human_review_required` all derive from
