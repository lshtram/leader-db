"""Flag-detection and rationale helpers for the domestic-violence scorer.

This module is the **internal** flag-detection logic of the
``domestic_violence`` deterministic scorer. The facade in
:mod:`leaders_db.score.domestic_violence` calls
...
The module mirrors the political-freedom scorer's flag-detection
...
as the strongest signal, and the client-source filter is applied
to the ``MissingObservation`` rollup so a contaminated bundle
...
from ._domestic_violence_rubric import _SPARSE_OBSERVED_RATIO_THRESHOLD
...
def _filter_excluded_missing(
    ...
    of
    :func:`leaders_db.score._domestic_violence_components.filter_excluded_observations`
...
def detect_flags(
    ...
    *,
    ...
    :func:`score_domestic_violence`) is derived from these
...
def build_rationale(
    *,
    ...
    not** state or imply a numeric score (the prior version
    interpolated ``score_1_10=1`` as a placeholder, producing the
    misleading sentence "Domestic violence score 1/10 on the
    1..10 prototype scale ..." for every insufficient-data
    row). The reviewer's blocker was that this contradicts the
    contract: a manual-review reader cannot tell from the
    rationale text whether a real score was emitted. The
    insufficient-data sentence is the canonical signal ("Bundle
    fell below the plan's minimum-viable source count; no score
    ...
    # mislead the manual-review reader; suppress it entirely and
    # rely on the "no score emitted" line below. ``normalized`` /
    # ``score_1_10`` are still required parameters so the scored
    # path keeps its single-call signature; we simply don't use
    # them on the insufficient-data branch.
    if ReviewFlag.INSUFFICIENT_DATA not in flags:
        parts.append(
            f"Domestic violence score {score_1_10}/10 on the 1..10 "
    ...
    if ReviewFlag.MISSING_PRIMARY_SOURCE in flags:
        parts.append(
            ...
            "pts_amnesty_score, cirights_physint absent."
        )
