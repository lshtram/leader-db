"""Per-component helpers for the domestic-violence scorer.
...
:mod:`leaders_db.score._domestic_violence_flags`.
...
- :func:`map_normalized_to_1_10` — the half-up rounding to the
  ...
  wellbeing / political-freedom scorer's mapping so
  cross-category comparisons stay on one scale);
...
from ._domestic_violence_rubric import (
    _GROUP_BY_VARIABLE,
    _GROUP_WEIGHTS,
    CATEGORY_KEY,
)
...
def map_normalized_to_1_10(normalized: float) -> int:
    ...
    social-wellbeing / integrity / effectiveness / economic
    wellbeing / political-freedom scorer's mapping so a
    reviewer can compare scores across the matrix without
...
def resolve_leader_name(bundle: CategoryEvidenceBundle) -> str:
    ...
    could not place a ruler; for the domestic-violence rubric
...
def build_observation_ref(
    ...
    ``TemporalKind.NOT_AVAILABLE`` observations so the ref
    points at the bundle's target year.
    """
    obs_year = (
...
def filter_excluded_observations(
    ...
    :func:`score_domestic_violence` as the **single** observation
    ...
    bundle reaches the scorer. See AGENTS.md always-on rule #6.
    """
    return tuple(
