"""Economic wellbeing deterministic scorer (requirement §9, §6).

Fourth per-category deterministic scorer. The rubric is a
transparent 3-group split (per-capita prosperity / WDI scale &
openness & investment / BTI economic transformation). The
``minimum_viable_sources=1`` setting plus the
``SparseDataPolicy.INSUFFICIENT_DATA`` policy mirrors the
effectiveness scorer's contract end-to-end:

- :mod:`leaders_db.score.economic_wellbeing` (this file) — the
  public :func:`score_economic_wellbeing` facade and the formula
  / missingness / client-handling docstring.
- :mod:`leaders_db.score._economic_wellbeing_rubric` — group
...
- :mod:`leaders_db.score._economic_wellbeing_flags` — the
...
import path
``leaders_db.score.economic_wellbeing.score_economic_wellbeing``
is stable.

This module exposes :func:`score_economic_wellbeing`, which
...
:mod:`leaders_db.score.category_plans.economic_wellbeing`.

Formula
-------

Three-group weighted-average rubric; group weights sum to 1.0.
The 0.45 / 0.25 / 0.30 split reflects the strength of the
direct signal each source carries: per-capita prosperity is the
strongest direct economic-wellbeing signal (GDP / GNI per capita
are the canonical "how well-off is the average citizen"
indicators); WDI scale / openness / investment indicators support
the per-capita view with market size, trade, and foreign-
investment context; BTI is the biennial expert-coded cross-
validator (Q6 / Q7 / Q11).
...
social-wellbeing / integrity / effectiveness scorer's mapping so
a reviewer can compare scores across the matrix without
re-deriving per-category scaling.
...
- If the bundle falls below the plan's ``minimum_viable_sources``
  ...
  :func:`leaders_db.score._economic_wellbeing_flags.detect_flags`).
  This is the :class:`ECONOMIC_WELLBEING_PLAN` setting.
...
from ._economic_wellbeing_components import (
...
from ._economic_wellbeing_flags import (
    build_missingness_summary,
    build_rationale,
    count_proxy_observations,
    detect_flags,
)
from ._economic_wellbeing_rubric import _GROUP_WEIGHTS, CATEGORY_KEY
from .evidence import CategoryEvidenceBundle, SparseDataPolicy
from .results import ReviewFlag, ScoreResult

__all__ = ["CATEGORY_KEY", "score_economic_wellbeing"]


def score_economic_wellbeing(bundle: CategoryEvidenceBundle) -> ScoreResult:
    """Return a :class:`ScoreResult` for the economic wellbeing category.
    ...
    bundle:
        ...
        :data:`ECONOMIC_WELLBEING_PLAN` (the function does not
    ...
    # ECONOMIC_WELLBEING_PLAN uses INSUFFICIENT_DATA so the gate is
