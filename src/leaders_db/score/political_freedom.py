"""Political freedom deterministic scorer (requirement §9, §6).

Fifth per-category deterministic scorer. The rubric is a
transparent 3-group split (V-Dem democratic / liberal /
civil-liberties / BTI political transformation / RSF press
freedom). The ``minimum_viable_sources=2`` setting plus the
``SparseDataPolicy.INSUFFICIENT_DATA`` policy mirrors the
economic-wellbeing / effectiveness scorer's contract end-to-end:

- :mod:`leaders_db.score.political_freedom` (this file) — the
  public :func:`score_political_freedom` facade and the formula
  / missingness / client-handling docstring.
- :mod:`leaders_db.score._political_freedom_rubric` — group
...
- :mod:`leaders_db.score._political_freedom_flags` — the
...
import path
``leaders_db.score.political_freedom.score_political_freedom``
is stable.

This module exposes :func:`score_political_freedom`, which
...
:mod:`leaders_db.score.category_plans.political_freedom`.

Formula
-------

Three-group weighted-average rubric; group weights sum to 1.0.
The 0.50 / 0.30 / 0.20 split reflects the strength of the
direct political-freedom signal each source carries: V-Dem's
7-indicator polyarchy / liberal-democracy / civil-liberties
family is the strongest direct signal (it is the V-Dem v16
canonical multi-dimensional democracy composite); BTI 2026 is
the biennial expert-coded cross-validator; RSF is the press /
media-freedom sub-signal.
...
wellbeing scorer's mapping so a reviewer can compare scores
across the matrix without re-deriving per-category scaling.
...
- If the bundle falls below the plan's ``minimum_viable_sources``
  ...
  :func:`leaders_db.score._political_freedom_flags.detect_flags`).
  This is the :class:`POLITICAL_FREEDOM_PLAN` setting.
...
from ._political_freedom_components import (
...
from ._political_freedom_flags import (
    build_missingness_summary,
    build_rationale,
    count_proxy_observations,
    detect_flags,
)
from ._political_freedom_rubric import _GROUP_WEIGHTS, CATEGORY_KEY
from .evidence import CategoryEvidenceBundle, SparseDataPolicy
from .results import ReviewFlag, ScoreResult

__all__ = ["CATEGORY_KEY", "score_political_freedom"]


def score_political_freedom(bundle: CategoryEvidenceBundle) -> ScoreResult:
    """Return a :class:`ScoreResult` for the political freedom category.
    ...
    bundle:
        ...
        :data:`POLITICAL_FREEDOM_PLAN` (the function does not
    ...
    # POLITICAL_FREEDOM_PLAN uses INSUFFICIENT_DATA so the gate is
