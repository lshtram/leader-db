"""Per-component helpers for the international-peace scorer.
...
:mod:`leaders_db.score._international_peace_flags`.
...
- :func:`map_normalized_to_1_10` — the half-up rounding to the
  ...
  mapping so cross-category comparisons stay on one scale);
...
from ._international_peace_rubric import (
    _GROUP_BY_VARIABLE,
    _GROUP_WEIGHTS,
    CATEGORY_KEY,
)
...
def resolve_leader_name(bundle: CategoryEvidenceBundle) -> str:
    ...
    could not place a ruler; for the international-peace rubric
...
def filter_excluded_observations(
    ...
    :func:`score_international_peace` as the **single** observation
