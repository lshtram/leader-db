"""Flag-detection and rationale helpers for the international-peace scorer.

This module is the **internal** flag-detection logic of the
``international_peace`` deterministic scorer. The facade in
:mod:`leaders_db.score.international_peace` calls
...
The module mirrors the political-freedom / economic-wellbeing /
domestic-violence scorer's flag-detection shape:
:func:`detect_flags` returns the scored-path flag set
...
from ._international_peace_rubric import _SPARSE_OBSERVED_RATIO_THRESHOLD
...
def _filter_excluded_missing(
    ...
    of
    :func:`leaders_db.score._international_peace_components.filter_excluded_observations`
...
def detect_flags(
    ...
    *,
    ...
    :func:`score_international_peace`) is derived from these
...
def build_rationale(
    *,
    ...
    misleading sentence "International peace score 1/10 on the
    ...
    if ReviewFlag.INSUFFICIENT_DATA not in flags:
        parts.append(
            f"International peace score {score_1_10}/10 on the 1..10 "
    ...
    if ReviewFlag.MISSING_PRIMARY_SOURCE in flags:
        parts.append(
            ...
            "ucdp_state_based_fatalities absent."
        )
