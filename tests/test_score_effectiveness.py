"""Tests for the effectiveness deterministic scorer — happy-path contract.

These tests pin the happy-path contract of
:func:`leaders_db.score.effectiveness.score_effectiveness`:

- (a) A realistic bundle produces a :class:`ScoreResult` with the
      right category key, ISO3, year, leader, and 1..10 integer
      score.
- (b) The 3-group weighted-average rubric structure (per-group
      weights, group contribution math, normalized-score sum).
- (c) Missingness summary: ``total_expected`` /
      ``total_observed`` and the by_reason / by_severity rollups.

Component / observation-ref bookkeeping, the scale mapping, the
rationale, the leader fallback, the client-source boundary
exclusion, and the regression tests for client missing-row
contamination live in the sibling file
:mod:`tests.test_score_effectiveness_components`. Flag-detection
tests live in :mod:`tests.test_score_effectiveness_flags`.

Style invariants (per ``docs/process/coding-guidelines.md``): type
hints, no ``print()``, no ``TODO(debug)``, no scratch code.
"""

from __future__ import annotations

import pytest

from leaders_db.score.category_plans import EFFECTIVENESS_PLAN
from leaders_db.score.effectiveness import (
    CATEGORY_KEY,
    score_effectiveness,
)
from leaders_db.score.evidence import (
    MissingObservation,
    MissingReason,
    MissingSeverity,
)
from leaders_db.score.results import ScoreComponent, ScoreResult
from tests._effectiveness_factories import (
    effectiveness_make_bundle,
    realistic_effectiveness_observations,
)

# (a) Happy path


def test_score_effectiveness_emits_valid_result_for_realistic_bundle() -> None:
    """A realistic bundle produces a result with the right shape."""
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations()
    )
    result = score_effectiveness(bundle)

    assert isinstance(result, ScoreResult)
    assert result.category_key == "effectiveness"
    assert result.iso3 == "MEX"
    assert result.year == 2023
    assert result.leader_name == "Andrés Manuel López Obrador"
    assert result.normalized_score_0_1 is not None
    assert 0.0 <= result.normalized_score_0_1 <= 1.0
    assert result.system_proposed_score_1_10 is not None
    assert 1 <= result.system_proposed_score_1_10 <= 10
    # The result is not the insufficient-data variant.
    assert result.is_insufficient_data is False
    assert result.is_provisional is False
    # Client comparison is downstream — the scorer leaves the slot empty.
    assert result.score_delta_vs_client is None


def test_score_effectiveness_normalized_sums_to_group_contributions() -> None:
    """The normalized score equals the sum of component contributions."""
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations()
    )
    result = score_effectiveness(bundle)

    assert result.normalized_score_0_1 is not None
    total_contribution = sum(c.contribution_0_1 for c in result.components)
    assert total_contribution == pytest.approx(
        result.normalized_score_0_1, abs=1e-9
    )


def test_score_effectiveness_realistic_bundle_expected_score() -> None:
    """The realistic bundle produces the expected 1..10 score.

    The realistic fixture values:

    - WGI governance group (weight 0.45): mean of
      ``(0.60, 0.55, 0.65, 0.60, 0.50)`` = 0.58 → contribution
      0.45 × 0.58 = 0.261.
    - V-Dem governance / accountability group (weight 0.35):
      mean of ``(0.60, 0.55, 0.65, 0.55, 0.70)`` = 0.61 →
      contribution 0.35 × 0.61 = 0.2135.
    - BTI governance group (weight 0.20): mean of
      ``(0.50, 0.55)`` = 0.525 → contribution 0.20 × 0.525
      = 0.105.

    Sum: 0.261 + 0.2135 + 0.105 = 0.5795. The 1..10 mapping
    yields ``floor(1 + 9 × 0.5795 + 0.5)`` = ``floor(6.7155)``
    = 6.
    """
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations()
    )
    result = score_effectiveness(bundle)

    assert result.normalized_score_0_1 is not None
    assert result.normalized_score_0_1 == pytest.approx(0.5795, abs=1e-9)
    assert result.system_proposed_score_1_10 == 6


# ---------------------------------------------------------------------------
# (b) The 3-group weighted-average rubric
# ---------------------------------------------------------------------------


def test_score_effectiveness_per_group_weights_sum_to_group_weight() -> None:
    """Per-group weights sum to the group weight; total sums to 1.0.

    The three effectiveness groups are WGI (0.45), V-Dem (0.35),
    BTI (0.20). Within a group, ``per_obs_weight =
    group_weight / count_in_group`` so per-component weights
    inside the group sum to the group weight. The WGI group has
    5 indicators in the realistic fixture, the V-Dem group has
    5, and the BTI group has 2.
    """
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations()
    )
    result = score_effectiveness(bundle)

    by_group: dict[str, list[ScoreComponent]] = {}
    for component in result.components:
        by_group.setdefault(component.component_key, []).append(component)

    total_weight = 0.0
    for group_key, components in by_group.items():
        group_weight = sum(c.weight for c in components)
        total_weight += group_weight
        weights = {round(c.weight, 9) for c in components}
        assert len(weights) == 1, (
            f"group {group_key!r} has non-uniform weights: {weights}"
        )
    assert total_weight == pytest.approx(1.0, abs=1e-9)


def test_score_effectiveness_wgi_group_uses_mean_of_indicators() -> None:
    """The WGI governance group emits one component per indicator and
    its contribution equals ``group_weight * mean(indicator values)``.
    """
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations()
    )
    result = score_effectiveness(bundle)

    wgi_components = [
        c
        for c in result.components
        if c.component_key == "effectiveness__wgi_governance"
    ]
    assert len(wgi_components) == 5
    per_obs_weight = 0.45 / 5
    expected_mean = (0.60 + 0.55 + 0.65 + 0.60 + 0.50) / 5
    for component in wgi_components:
        assert component.weight == pytest.approx(per_obs_weight, abs=1e-9)
        assert component.contribution_0_1 == pytest.approx(
            component.normalized_value_0_1 * per_obs_weight, abs=1e-9
        )
    assert sum(c.contribution_0_1 for c in wgi_components) == pytest.approx(
        0.45 * expected_mean, abs=1e-9
    )


def test_score_effectiveness_vdem_group_uses_mean_of_indicators() -> None:
    """The V-Dem governance / accountability group emits one component
    per indicator and its contribution equals
    ``group_weight * mean(indicator values)``.
    """
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations()
    )
    result = score_effectiveness(bundle)

    vdem_components = [
        c
        for c in result.components
        if c.component_key
        == "effectiveness__vdem_governance_accountability"
    ]
    assert len(vdem_components) == 5
    per_obs_weight = 0.35 / 5
    expected_mean = (0.60 + 0.55 + 0.65 + 0.55 + 0.70) / 5
    for component in vdem_components:
        assert component.weight == pytest.approx(per_obs_weight, abs=1e-9)
    assert sum(c.contribution_0_1 for c in vdem_components) == pytest.approx(
        0.35 * expected_mean, abs=1e-9
    )


def test_score_effectiveness_bti_group_uses_mean_of_indicators() -> None:
    """The BTI governance group emits one component per indicator and
    its contribution equals ``group_weight * mean(indicator values)``.
    """
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations()
    )
    result = score_effectiveness(bundle)

    bti_components = [
        c
        for c in result.components
        if c.component_key == "effectiveness__bti_governance"
    ]
    assert len(bti_components) == 2
    per_obs_weight = 0.20 / 2
    expected_mean = (0.50 + 0.55) / 2
    for component in bti_components:
        assert component.weight == pytest.approx(per_obs_weight, abs=1e-9)
    assert sum(c.contribution_0_1 for c in bti_components) == pytest.approx(
        0.20 * expected_mean, abs=1e-9
    )


# ---------------------------------------------------------------------------
# (c) Missingness summary
# ---------------------------------------------------------------------------


def test_score_effectiveness_missingness_total_matches_plan() -> None:
    """``total_expected`` equals the plan's indicator count."""
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations()
    )
    result = score_effectiveness(bundle)

    assert result.missingness is not None
    assert result.missingness.total_expected == len(
        EFFECTIVENESS_PLAN.expected_indicators
    )
    # We observed all 12 plan variables in the realistic fixture.
    assert result.missingness.total_observed == 12
    assert (
        result.missingness.total_missing
        == result.missingness.total_expected - result.missingness.total_observed
    )


def test_score_effectiveness_missingness_rolls_up_missing_observations() -> None:
    """``by_reason`` and ``by_severity`` count the bundle's missing rows."""
    missing = [
        MissingObservation(
            source_key="wgi",
            variable_name="wgi_government_effectiveness",
            reason=MissingReason.RAW_FILE_ABSENT,
            severity=MissingSeverity.PRIMARY,
        ),
        MissingObservation(
            source_key="bti",
            variable_name="bti_governance_index",
            reason=MissingReason.TARGET_YEAR_ABSENT,
            severity=MissingSeverity.PRIMARY,
        ),
        MissingObservation(
            source_key="vdem",
            variable_name="vdem_v2x_mpi",
            reason=MissingReason.COUNTRY_ROW_ABSENT,
            severity=MissingSeverity.IMPORTANT,
        ),
    ]
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations(), missing=missing
    )
    result = score_effectiveness(bundle)

    assert result.missingness is not None
    assert dict(result.missingness.by_reason) == {
        "raw_file_absent": 1,
        "target_year_absent": 1,
        "country_row_absent": 1,
    }
    assert dict(result.missingness.by_severity) == {
        "primary": 2,
        "important": 1,
    }


def test_score_effectiveness_category_key_is_canonical() -> None:
    """The category_key on the result is the canonical ``effectiveness``."""
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations()
    )
    result = score_effectiveness(bundle)
    assert result.category_key == CATEGORY_KEY == "effectiveness"
