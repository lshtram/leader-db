"""Tests for the domestic-violence scorer's insufficient-data branch.

These tests close the reviewer blocker "flag completeness on
insufficient-data path" for the domestic-violence scorer. The
...
insufficient-data path, producing "Domestic violence score
...
:mod:`tests.test_score_stage9_domestic_violence_batch`.

The split mirrors the production code split
(:mod:`leaders_db.score.domestic_violence` (facade) +
:mod:`leaders_db.score._domestic_violence_flags` (flag
helpers)) and the existing test split
(:mod:`tests.test_score_domestic_violence_flags` covers the
scored-path flag derivation;
:mod:`tests.test_score_domestic_violence_remediation` covers
...
# Pattern that catches a numeric-score sentence such as "Domestic
# violence score 1/10 ..." or any "score N/10" interpolation. The
...
def test_insufficient_data_derives_missing_primary_flag() -> None:
    ...
    bundle = domestic_violence_make_bundle(missing=missing)
    ...
    # The rationale mentions the canonical PTS REQUIRED so a
...
def test_insufficient_data_derives_low_confidence_flag_for_proxy() -> None:
    ...
    # below ``minimum_viable_sources=2`` (the domestic-violence
...
def test_insufficient_data_client_missing_rows_no_primary_flag() -> None:
    ...
    bundle = domestic_violence_make_bundle(missing=only_client_missing)
    result = score_domestic_violence(bundle)
...
# placeholder for the rationale text only, producing "Domestic
# violence score 1/10 on the 1..10 prototype scale ..." for every
# insufficient-data row. The fix suppresses the numeric-score
# sentence on the insufficient-data path so the rationale carries
# only the canonical gate-signal text. The Stage 9 / CSV-facing
# proof for the same contract lives in
# :mod:`tests.test_score_stage9_domestic_violence_batch`.
...
def test_insufficient_data_rationale_does_not_state_numeric_score() -> None:
    ...
    rationale that started with "Domestic violence score 1/10
