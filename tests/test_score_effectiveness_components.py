"""Tests for the effectiveness deterministic scorer — component / ref /
...
:func:`leaders_db.score.effectiveness.score_effectiveness`. The
...
regression test in :mod:`tests.test_score_effectiveness_remediation`.
...
def test_score_effectiveness_one_component_per_observation() -> None:
    """Every usable observation produces exactly one ScoreComponent."""
    observations = realistic_effectiveness_observations()
    bundle = effectiveness_make_bundle(observations=observations)
...
def test_score_effectiveness_components_carry_observation_refs() -> None:
...
def test_score_effectiveness_observation_refs_flat_mirrors_components() -> None:
...
def test_score_effectiveness_group_keys_match_rubric() -> None:
    """Every component's ``component_key`` is prefixed with the group key.

    The effectiveness rubric emits
    ``effectiveness__<group_key>`` for each per-observation
    :class:`ScoreComponent`. The three group keys are
    ``wgi_governance``, ``vdem_governance_accountability``, and
    ``bti_governance`` per the rubric constants in
    :mod:`leaders_db.score._effectiveness_rubric`.
    """
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations()
    )
    ...
    expected_prefixes = {
        "effectiveness__wgi_governance",
        "effectiveness__vdem_governance_accountability",
        "effectiveness__bti_governance",
    }
    actual_prefixes = {c.component_key for c in result.components}
    assert actual_prefixes == expected_prefixes


def test_score_effectiveness_observes_three_distinct_sources() -> None:
    """The realistic fixture spans all 3 expected sources (wgi, vdem, bti).

    The dispatcher test for ``effectiveness`` (in
    :mod:`tests.test_score_dispatch`) requires the scorer to
    receive all 3 sources so it can produce the canonical
    ...
    assert component_source_keys == {"wgi", "vdem", "bti"}
...
def test_score_effectiveness_rationale_short_mentions_scale() -> None:
...
def test_score_effectiveness_score_clamped_to_1_to_10() -> None:
...
def test_score_effectiveness_leader_name_falls_back_to_country() -> None:
    """If the bundle has no leader, the result uses the country name."""
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations(), leader_name=None
    )
    result = score_effectiveness(bundle)
...
def test_score_effectiveness_observation_with_none_normalized_is_skipped() -> None:
    """Observations with ``normalized_value=None`` do not contribute.

    The bundle pairs a ``wgi`` source with one null observation,
    plus a second source (vdem + bti) with usable observations.
    ...
    for component in result.components:
        assert not (
            ...
            and component.variable_name == "wgi_government_effectiveness"
        )
    for ref in result.observation_refs:
        assert not (
            ref.source_key == "wgi"
            and ref.variable_name == "wgi_government_effectiveness"
        )
    assert len(result.components) == 2
    assert len(result.observation_refs) == 2


def test_score_effectiveness_is_deterministic() -> None:
    """Two calls with the same bundle produce the same score."""
    bundle = effectiveness_make_bundle(
        observations=realistic_effectiveness_observations()
    )
    first = score_effectiveness(bundle)
    second = score_effectiveness(bundle)
...
def test_score_effectiveness_does_not_consult_client_observations() -> None:
    ...
    :mod:`tests.test_score_effectiveness_remediation`.
    """
    contaminated_observations = [
        EvidenceObservation(
            ...
            target_year=2023,
            temporal_kind=TemporalKind.DIRECT,
            source_row_reference="client_existing:wgi_government_effectiveness:2023",
            authority_score=70,
            specificity_score=80,
        ),
        effectiveness_make_obs("vdem_v2x_accountability", "vdem", 0.65),
    ...
    bundle = effectiveness_make_bundle(observations=contaminated_observations)
    result = score_effectiveness(bundle)
