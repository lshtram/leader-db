"""Tests for the domestic-violence deterministic scorer — reviewer-blocker
regression tests.

These tests pin the regression fix for the reviewer blocker
"client-contamination / missingness correctness" on the
domestic-violence scorer. The domestic-violence scorer's flag-
...
- :mod:`tests.test_score_domestic_violence_components` — per-
  component bookkeeping + scale mapping + rationale + leader
  fallback + determinism + per-observation client exclusion;
- :mod:`tests.test_score_domestic_violence_remediation` — this
...
# missingness correctness". The domestic-violence scorer's flag-
# detection and missingness-rollup paths must apply the same
# client-source filter the observation path applies: a
# contaminated bundle carrying ``MissingObservation`` rows whose
# ``source_key`` is in ``EXCLUDED_SOURCE_KEYS``
...
def test_score_domestic_violence_client_missing_rows_do_not_inflate_missingness() -> (
    None
):
    ...
    """
    # Three non-client missing rows: two PRIMARY (the PTS and
    # CIRIGHTS REQUIRED indicators) and one IMPORTANT (a UCDP
    # preferred).
    clean_missing = [
        MissingObservation(
            source_key="pts",
    ...
    clean_bundle = domestic_violence_make_bundle(
        observations=obs, missing=clean_missing
    )
    contaminated_bundle = domestic_violence_make_bundle(
        observations=obs, missing=[*clean_missing, *client_missing]
    )

    clean_result = score_domestic_violence(clean_bundle)
    contaminated_result = score_domestic_violence(contaminated_bundle)
    ...
    #    change which flags fire. Both bundles carry the
    #    PTS + CIRIGHTS PRIMARY missing → MISSING_PRIMARY_SOURCE.
...
def test_score_domestic_violence_only_client_missing_rows_suppress_primary_flag() -> (
    None
):
    ...
    """
    obs = realistic_domestic_violence_observations()
    only_client_missing = [
        MissingObservation(
            source_key="client_existing",
            variable_name="pts_amnesty_score",
    ...
    clean_bundle = domestic_violence_make_bundle(observations=obs)
    ...
    # The clean bundle observes all 17 plan variables and has no
...
def test_score_domestic_violence_client_missing_rows_skipped_in_insufficient_path() -> (
    None
):
