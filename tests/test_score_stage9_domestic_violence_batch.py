"""Stage 9 all-countries batch seam tests for the ``domestic_violence`` category.

Sibling of :mod:`tests.test_score_stage9_domestic_violence` (the
single-country seam + sparse-bundle insufficient-data proof) and
:mod:`tests.test_score_stage9_batch` (the canonical social-wellbeing
batch-seam proof shared by every per-category seam). Splitting the
batch-seam proof into its own focused module keeps the single-country
file under the 400-line convention while still being one test module
per production seam family — same pattern as
...
``MEX`` 2023 with the dense domestic-violence bundle (PTS +
CIRIGHTS + UCDP + V-Dem, 17 indicators across 4 sources) plus
``BRA`` 2023 with no observations (a clean insufficient-data
result), then asserts the batch returns one :class:`ScoreResult`
per country in ``iso3`` order — ``BRA < MEX`` lexicographically —
so the deterministic order is independent of insertion order.

A CSV-facing proof pins the reviewer-blocker remediation that
insufficient-data ``rationale_short`` strings do **not** state or
imply a numeric score. The Stage 9 CSV writer
...
from .test_score_stage9_domestic_violence import (
...
# (``BRA < MEX`` lexicographically). Mirrors the social-wellbeing
# batch-seam sibling.
...
def _seed_mexico_and_brazil(database_url: str) -> None:
    """Seed MEX (dense domestic_violence bundle) + BRA (no observations).

    Mirrors the social-wellbeing batch-seam helper. The seed loop
    inserts ``BRA`` after ``MEX`` so the iso3-ordering assertion
    below exercises the deterministic sort, not just the
    insertion order of the seed loop.
    """
...
def test_score_category_for_all_countries_domestic_violence_returns_one_per_country(
    database_url: str,
) -> None:
    ...
    for r in results:
        ...
        assert r.category_key == "domestic_violence"
        assert r.year == TARGET_YEAR


def test_score_category_for_all_countries_domestic_violence_scored_country_has_score(
    database_url: str,
) -> None:
    ...
    # 17 indicators × 1 ref each = 17 refs total.
    assert len(mex.observation_refs) == 17
    assert mex.review_flags == ()
    assert mex.human_review_required is False


def test_score_category_for_all_countries_domestic_violence_missing_country_has_insufficient(
    database_url: str,
) -> None:
    ...
    # because observed_ratio (0/17) is below the 0.5 threshold.
    ...
    assert bra.missingness.total_expected == 17
...
def test_write_score_results_csv_domestic_violence_insufficient_rationale_has_no_numeric_score(
    database_url: str,
    tmp_path: Path,
) -> None:
    ...
    path, producing "Domestic violence score 1/10 on the 1..10
    ...
    output_path = write_score_results_csv(
        results, tmp_path / "domestic_violence.csv"
    )
