...
21. **Obtain explicit approval before every API-key-backed model run.** The default model execution surface is the existing Codex/subscription workflow. Before using `OPENAI_API_KEY`, another provider credential, or any directly billed model API—even for a diagnostic, cache probe, quality review, retry, or experiment—the agent must notify the user and obtain explicit approval for a bounded run. The approval request must state the specific provider and model, purpose, maximum number of calls, estimated input/cached-input/output token quota, estimated cost and hard spending ceiling, expected artifacts, and whether outputs are diagnostic or production-eligible. Do not launch calls, reserve work, or treat a repository credential as authorization before approval. Approval applies only to the stated model, quota, purpose, and run; increases, retries beyond the approved allowance, model substitutions, and later phases require renewed approval. Record approved limits and actual usage in the run manifest. If the subscription surface cannot expose a required metric, report that limitation and ask whether the user approves a bounded API diagnostic; do not switch surfaces silently.
22. **Fix the general pipeline, never a failed test run.** Treat every failed or partial run as an immutable diagnostic. Diagnose what general contract, validator, scheduler, transport, or orchestration rule allowed the failure; implement and test that reusable rule independently of the case; then evaluate it in a completely fresh run directory and release identity. Never edit, normalize, resume, relabel, or selectively repair a failed run into a passing artifact. A failed case may be used only as an explicitly labeled prototype or regression fixture. Any exception, normalization, retry allowance, concurrency policy, or budget rule must be defined prospectively for all applicable future runs, preserve raw outputs and cumulative usage, and receive focused tests and independent review before a fresh evaluation begins.
