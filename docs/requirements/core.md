...
- **REQ-LLM-020**: Ruler dossier production shall separate candidate discovery, evidence acceptance, and dossier serialization. One researcher shall receive the ruler-period, selected research lenses, and local-prior package and work through chapters 1B–8B sequentially in one session. It shall maintain a lightweight candidate log (query, title, URL, status) separately from one append-only accepted-evidence ledger. Accepted records carry a stable canonical source-locator-claim key, exact URL, locator when recoverable, main-points summary, period and ruler fit, contrary material, and many-to-many chapter/lens relevance. A substantial report may support multiple materially distinct, separately located claims; source concentration is reported rather than capped per URL. A minimal stable-key/disposition manifest shall make ledger-to-dossier preservation deterministic without duplicating content. Gateway-only and underlying-source-missing candidates do not count as defensible evidence; an identified locator-missing source remains a recoverable extraction task. A reviewer may return concrete discovery, fetch, locator, attribution, or contrary-evidence gaps for up to three rounds. A separately configured formatter shall emit the validated dossier without new research, preserve every ledger item or explicitly reject it, consolidate only duplicate facts, and map every retained item. Coverage is derived from mappings. The goal is 5–20 filtered source-claim units per chapter, not a search-results ceiling. A sparse chapter remains publishable when inspected candidates, rejection reasons, and exact gaps are documented; no component may pad, invent evidence, or give score/ranking advice during research.
- **REQ-LLM-021**: Evidence-quality review shall inspect all selected chapters without scoring them. It shall distinguish genuine scarcity from shallow collection, identify recoverable substantive gaps, and return a deficient chapter only when another web round is reasonably likely to add the configured minimum of new defensible precisely located units or close a material ruler-attribution blocker. Deduplication, claim splitting, neutral wording, lens dispositions, count reconciliation, and serialization are deterministic cleanup and shall not trigger further web research. Review and resumption stop on a pass, a credible saturation/access blocker, failure of the marginal-value gate, or the configured maximum of three rounds.
