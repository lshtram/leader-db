...
- **REQ-LLM-020**: Ruler dossier production shall separate evidence research from dossier serialization. One researcher shall receive the complete guides, ruler-period, and local-prior package, work through chapters 1B–8B sequentially, and search directly until reasonably saturated. It shall maintain a permissive evidence notebook with exact references, main-points summaries, period and ruler fit, contrary material, and candidate chapter/lens relevance; it shall not be constrained by the final JSON schema. An evidence reviewer may return specific gaps to the same researcher session for up to three rounds. A separately configured formatter shall emit the validated dossier without new research. It shall preserve materially distinct source-claim units even when they share a URL, retain cross-round reviewer evidence estimates as formatting targets, and reject material evidence loss; recovery shall rank only structurally valid, internally linked candidates. The goal is 5–20 defensible source-claim units per chapter (normally about 10), with justified many-to-many reuse. A sparse chapter remains publishable when inspected candidates, rejection reasons, and exact gaps are documented; no component may pad or invent evidence.
- **REQ-LLM-021**: Evidence-quality review shall inspect all selected chapters without scoring them. It shall distinguish genuine scarcity from shallow collection, identify recoverable substantive gaps, and return every deficient chapter to the same researcher session. Review and resumption stop on a pass, a credible saturation/access blocker, or the configured maximum of three rounds. Mechanical count/ID repair remains a formatter responsibility.
