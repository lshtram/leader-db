...
- **REQ-LLM-018**: One persistent researcher session shall receive one resolved ruler-period, the complete selected chapter guides, and the locally extracted fact package. It shall work through chapters in order and search the internet directly and iteratively until each chapter is reasonably saturated or further improvement is blocked. An evidence reviewer shall inspect every selected chapter and may return any deficient chapters to the same researcher session for up to three review rounds. Research stops earlier when the reviewer passes the dossier or the researcher gives a credible saturation/access-blocker explanation. No preselected link packet, fixed query allowance, or three-chapter continuation cap may substitute for researcher-directed search.
...
- **REQ-LLM-020**: Ruler dossier production shall separate evidence research from dossier serialization. One researcher shall receive the complete guides, ruler-period, and local-prior package, work through chapters 1B–8B sequentially, and search directly until reasonably saturated. It shall maintain a permissive notebook plus one append-only global evidence ledger whose new records carry a stable canonical source-locator-claim key, exact URL, precise page/section/paragraph/table/timestamp or local-prior locator, main-points summary, period and ruler fit, contrary material, and many-to-many chapter/lens relevance. Gateway-only, underlying-source-missing, and locator-missing candidates shall not count as defensible evidence. An evidence reviewer may return specific gaps to the same researcher session for up to three rounds. A separately configured formatter shall emit the validated dossier without new research. It shall preserve materially distinct source-claim units even when they share a URL, consolidate duplicate facts across chapters, retain cross-round reviewer evidence estimates as proportional formatting targets, and reject material evidence loss without inventing a minimum floor above the reviewer's defensible estimate; recovery shall rank only structurally valid, internally linked candidates. The goal is 5–20 defensible source-claim units per chapter (normally about 10), with justified many-to-many reuse. A sparse chapter remains publishable when inspected candidates, rejection reasons, and exact gaps are documented; no component may pad, invent evidence, or give score/ranking advice during research.
