...
- **REQ-LLM-022**: Every full-ruler research job shall begin with parent-side, client-matrix-excluding local evidence extraction for all selected `1B`–`8B` lenses. The parent shall preserve the complete hashed per-lens prior artifact and provide both researcher and formatter a versioned deduplicated package containing stable local fact IDs, exact source observation IDs, valid local-prior locators, per-methodology statuses and deduplicated missing/error dispositions, warnings, mapping notes, and candidate chapter/lens links. Identical facts shall appear once even when relevant to many lenses. Candidate links are routing hints rather than automatic claim mappings; missing rows are evidence gaps rather than zero/favorable observations, local extraction errors are blocking data-quality failures rather than ordinary missingness, and country indicators require explicit inherited-baseline and ruler-attribution treatment before use.
...
- **REQ-LLM-023**: A notebook-mode ruler job shall persist the exact Codex thread ID from the initial researcher event stream. Each reviewer-directed continuation shall resume that exact thread and allow direct searches for the identified gaps; concurrent jobs shall never use `--last` session selection. All phase prompts, outputs, events, hashes, provider/model identities, and usage shall remain attempt-scoped and auditable.
