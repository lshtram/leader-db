...
| `/reports/leaders/` | nginx static files | Leaders Database year selector |
| `/reports/leaders/2023/` | year-routed bind mount from `docs/client-results/2023-top20/` | 2023 client-score comparison and evidence viewer |
| `/reports/leaders/2024/` | year-routed bind mount from `docs/client-results/2024-top20/` | 2024 evidence-based scores and rationale viewer |
| `/reports/leaders-2023/` | permanent redirect | Compatibility URL for the original 2023 publication |
...
docs/client-results/                                         -> /usr/share/nginx/html/leaders-releases
...
- the Leaders Database year registry and both 2023/2024 viewer, JSON, and attribution
...
### Add a Leaders Database year

1. Create the immutable release directory at
   `docs/client-results/<year>-top20/` with `index.html`, `viewer.css`, `viewer.js`,
   `data.json`, and `attributions.md`.
2. Add the year and canonical `/reports/leaders/<year>/` path to
   `infra/superset/reports/leaders/years.json`.
3. Add the new year routes to `scripts/check_viz_reports.py`.
4. Recreate `superset-proxy` and run the local health check. No nginx location or
   compose mount is needed for each additional year.
