...
| `pwt` | ✅ | Penn World Table 10.01 | Free xlsx, 6.5MB; 183 economies, 1950–2019, PPP-based; cross-validates WDI. Raw file is staged at `data/raw/pwt/pwt1001.xlsx` with `metadata.json`; Stage 2 adapter is not yet implemented. For target years beyond 2019, PWT must not silently proxy/stale-fill rows; out-of-coverage requests should emit zero observations plus a manifest warning. |
