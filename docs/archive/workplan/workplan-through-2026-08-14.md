...
## 2026-08-13 — Straight-through 4B chapter test completed; quality failed

- Implemented and independently reviewed a simple two-phase chapter path: ten ordered
  Terra question writes followed by ten separate Sol reviews. There is no internal
  critique, repair planner, correction pass, or retry loop. Reused directories, altered
  answers, wrong baselines, path escapes, non-Codex profiles, and manifest changes fail
  deterministic validation before additional calls.
- The real Netanyahu 4B writing phase completed all ten questions on the first attempt.
  It used 10 calls, 554,192 input tokens (80,640 reported cached), and 18,502 output
  tokens. The corrected blind-review phase used 10 calls, 588,982 input tokens (55,040
  reported cached), and 12,450 output tokens. All calls used the Codex subscription; no
  API key was used.
- The chapter quality gate failed: none of the ten experimental answers was at least
  equal to the exact approved baseline under every required dimension. The main repeated
  cause was input selection. Exact evidence was inherited from citations in the approved
  answer, while other directly relevant records were available only as compact summaries;
  the reviewer repeatedly identified material compact-index records that should have been
  exact writer input. Secondary problems included incomplete inherited-baseline treatment,
  isolated citation overreach, and ambiguous evidence-role labels whose meaning could be
  read as supporting a fact, supporting the ruler, or supporting the answer's conclusion.
- No answer repair or re-review was started. Preserve the simple one-write/one-review
  phase boundary. The next development decision is upstream: select a fuller exact
  evidence set before writing and define evidence roles unambiguously, then rerun the
  frozen chapter once. Do not solve these omissions with downstream review loops.

## 2026-08-13 — Contrasting Terra question test passed

- Tested 4B.4, the broadest 4B packet by candidate count, using 11 exact required
  records and a compact index of 155 candidates. The experiment used one Terra writing
  call and one independent Sol review, with no repair or retry.
- Terra passed every mechanical check and the reviewer preferred its answer. It passed
  factual support, citation fit, material coverage, balance, attribution, period handling,
  and judgeability with no unsupported claim or material regression. In particular, it
  incorporated the wartime court-emergency evidence and treated the war as an external
  shock without overstating Netanyahu's personal responsibility.
- Usage was 71,237 input and 1,403 output tokens for writing, followed by 74,100 input
  tokens (11,008 reported cached) and 1,057 output tokens for review. Both calls used the
  Codex subscription; no API key was used.
- With difficult questions in both 4B and 8B now passing, the next implementation unit is
  a simple ten-question chapter runner: one Terra write per question, deterministic
  validation, and a separate review phase. It should contain no internal critique,
  correction, repair-planning, or retry loop.

## 2026-08-13 — Terra question-writer comparison passed

- Repeated the frozen 8B.9 test with the same 20 exact records and unchanged writer
  instructions, replacing Luna with Terra. The test used one Terra writing call and one
  independent Sol blind-review call; no repair or retry followed.
- Terra cited and dispositioned all 20 required records. The blind reviewer preferred
  its answer and passed factual support, citation fit, material coverage, balance,
  attribution, period handling, and usefulness, with no material regression or
  unsupported claim. It restored the omitted civilian-assistance and Prime Minister's
  Office evidence while preserving the earlier answer's qualifications.
- Usage was 116,287 input and 2,457 output tokens for writing, followed by 119,757 input
  tokens (11,008 reported cached) and 1,169 output tokens for review. Both calls used the
  Codex subscription; no API key was used.
- This is one difficult question, not a chapter-level promotion. The next bounded test
  should use the same one-write/one-review Terra path on a contrasting 4B question before
  deciding whether to build the complete ten-question chapter runner.

## 2026-08-13 — Targeted question reopen improved coverage but did not pass

- Applied one exact wording correction to the diagnostic 8B.9 answer without another
  writing call. The blind reviewer found the unsupported wording resolved and preferred
  the corrected answer, but found that both the approved and experimental answers omitted
  the same material evidence about wartime civilian assistance, adaptive improvements,
  and Prime Minister's Office coordination.
- Reopened only those four exact records, made one Luna writing call, and made one Sol
  blind-review call. The expanded answer covered all 20 required records and was preferred
  overall because it was materially more complete and balanced.
- The expanded answer did not pass: it incorrectly associated a pre-pause hostage rescue
  with the November pause and attached a starvation-related legal characterization to
  citations that did not themselves establish it. These are citation-precision errors,
  despite the coverage improvement.
- No additional correction or review loop was started. The result remains diagnostic-only.
  Before scaling question-level writing, the next design decision should compare a more
  precise writer or a simpler claim-level instruction against this frozen case, with one
  write and one review rather than repeated repair. All calls used the Codex subscription;
  no API key was used.

## 2026-08-13 — Single-question writer gate hardened; quality promotion blocked

- Tested the replacement writer on Netanyahu question 8B.9, a high-risk question with
  16 exact required records and 205 indexed candidates. The first attempt failed because
  it listed every citation but discussed only two subjects. That result remains a
  diagnostic and is not approved output.
- Added a required per-source explanation. The bounded Luna retry discussed all 16 exact
  records, passed the automatic identity, citation, reopening, and source-accounting
  checks, and introduced no unknown evidence IDs.
- An initial blind review preferred the new answer, but independent code review found
  that the saved gate could trust contradictory review fields and was not bound to its
  exact inputs. The corrected gate now fails closed and hashes the packet, both answers,
  prompt, and review. Under that corrected gate, a fresh answer failed because its final
  phrase said warnings were "ignored," while the evidence establishes warnings and no
  measured correction but not that they were ignored in full. This answer is diagnostic
  only and is not promoted.
- All model calls used the Codex subscription; no API key was used. Next gate: add a
  bounded factual-overstatement correction step and reopen only the strongest omitted
  candidates, then require the same hash-bound blind comparison to pass before rollout.

## 2026-08-13 — Deterministic question-evidence packets implemented

- Added a no-model projection that gives each question exact priority evidence from its
  selected approved answer plus a compact index of all other directly routed candidates.
  Every frozen-ledger record receives one auditable chapter disposition; client values
  are not read.
- The Netanyahu smoke build passes for 4B and 8B. Exact priority context ranges from
  6–20 records per question instead of sending 282 or 447 undifferentiated chapter
  candidates. The compact candidate indexes remain available for omission checks and
  targeted reopening, including the largest 8B.9 index of 205 records.
- This phase does not alter answer generation and makes no model call. The next gate is
  to add deterministic coverage requirements and test one question writer against the
  approved baseline before changing the chapter workflow.

## 2026-08-13 — Question coverage gate ready for writer testing

- Added a deterministic checklist to every question packet. Every approved citation is
  marked `must_address`; every other directly routed candidate remains
  `available_for_reopen`. Each item records whether ruler attribution, period fit, and
  limitations are present, and each question declares the available favorable, adverse,
  and mixed/context evidence without treating a missing polarity as evidence.
- The real Netanyahu 4B/8B rebuild passes: all approved citations are present as exact
  evidence, and every compact candidate has one checklist disposition. Priority context
  remains 6–20 records per question while the full candidate index stays available for
  omission challenges.
- The established chapter writer remains unchanged. The next development cycle can run
  one diagnostic question writer, beginning with a high-risk lens, followed by automatic
  citation/coverage checks and blind comparison with the approved answer. No wider
  rollout is eligible until that single-question quality gate passes.

## 2026-08-13 — Unsuccessful request-envelope mechanism removed

- Repeated Netanyahu 4B/8B experiments showed that reorganizing the same large evidence
  request did not improve quality reliably. The Codex subscription surface reported no
  repeated-input cache reuse, and blind review found important omissions, weak citation
  matches, and occasional overstatement.
- Removed the alternate request builder, its opt-in runner integration, configurations,
  scripts, and tests. Restored the established chapter-analysis implementation exactly;
  failed run artifacts remain immutable diagnostics and never entered production.
- The next optimization phase will reduce undifferentiated model input by building
  deterministic question-level evidence packets and coverage manifests before chapter
  writing. No API key will be used without separate approval.

## 2026-08-13 — Canonical model-cost profiling implemented

- Implemented Step 1 of the model-cost optimization plan as an isolated,
  configuration-driven profiler. It classifies trusted event artifacts by explicit
  artifact type, separates selected, superseded, and failed work, and emits a stable
  JSON manifest, CSV call ledger, and Markdown report/comparison.
- Froze fresh profiles for the approved Netanyahu v4 and earlier deep-corpus packages
  under `research/runs/netanyahu-2023-cost-opt-step01-profile-v1/`. Both snapshots bind
  all eight selected chapters and exactly eighty answers without changing scientific
  artifacts or making model calls. Step 2 has not begun.

## 2026-08-13 — Public five-ruler study microsite implemented

- Added a deterministic public projection and static-site builder for the approved
  `2023-five-ruler-flow-test-v2` run. Publication requires an allowed audit, five
  approved rulers, eight judged chapters per ruler, ten evidence questions per chapter,
  and resolvable cited evidence IDs.
- The generated editorial site presents results, ruler judgments, all eighty question
  answers, source-grouped verified excerpts, methodology, pipeline, limitations, and
  run details. Chapter remains the scoring unit; question scores and client-matrix
  values are excluded. Equal overall means receive shared ranks.
- Added `research build-study-site` and `research check-study-site`. The real build emits
  a hash manifest covering 701 files for `/leaders-study/` on `www.chopsworkshop.com`.
...
## Open design concern — review-loop complexity (2026-08-13)

- A read-only audit found that the main research path is understandable at the phase
  level, but several phases contain their own review-and-correction loops. Research can
  be reviewed, resumed, reviewed again, and transferred to a supervisor. Chapter writing
  can include an internal critique, independent quality review, repair planning, answer
  repair, compaction, and another review. Some individual formatting or planning calls
  also have several correction attempts.
- These controls were introduced in response to real quality failures, so they should not
  be removed merely to simplify the diagram or reduce calls. The open question is whether
  the same evidence quality can be maintained with clearer boundaries: one production
  phase, one dedicated independent quality phase, and at most one targeted return for a
  material defect. Mechanical schema, identity, citation, provenance, and transport
  validation should remain deterministic and should not become additional model-review
  phases.
- The optimization plan itself also applies a large blind-review and adversarial-audit
  protocol after every model-bearing development step. That is appropriate for major
  promotion decisions but may be disproportionate for small experiments. A future design
  pass should distinguish development diagnostics from production quality phases and
  decide where full comparative review is genuinely required.
- No pipeline behavior has been changed as a result of this finding. Continue the current
  development path while preserving this as an explicit simplification objective. Before
  removing any loop, compare quality and call counts on the same frozen case; retain the
  loop if the simpler path materially weakens factual support, coverage, balance,
  attribution, or judgeability.

## Straight-through Chapter 4B result — 2026-08-14

- The Netanyahu 2023 Chapter 4B trial now passes all ten blind question reviews. The
  successful immutable artifacts are under
  `research/runs/netanyahu-2023-cost-opt-step03-chapter-v5/4B/`: ten Terra-written
  answers in `writing/` and the trusted, deterministically rebound Sol comparisons in
  `review-v4/`.
- The useful upstream correction was small: give each writer the approved predecessor,
  promote the reviewer-identified omitted evidence to exact records, and show the writer
  only candidates that are still eligible for reopening. The resulting ten answers all
  passed deterministic identity and evidence-completeness checks.
- Two redundant output mechanisms were removed during the cycle. Reopen IDs and reasons
  are now one structured request, and evidence dispositions are the single citation
  ledger instead of being synchronized with two extra citation arrays. These changes
  removed mechanical failures without adding a review or repair phase.
- The blind gate now reflects the reviewer's substantive conclusion: all six quality
  dimensions must pass, unsupported claims must be absent, and the experimental answer
  must be preferred or tied. Bounded tradeoff comments remain visible but no longer
  override an explicit preference. On the final evidence set, all ten answers meet this
  rule; no additional model review was needed to apply the corrected deterministic gate.

## Chapter-scoped review experiment rejected — 2026-08-14

- Tested whether the ten independent Sol question reviews could become one chapter call.
  No model call was made. The complete reconstructed 4B request was 2,005,969 characters.
  Deduplicating repeated evidence into one shared chapter table reduced it to roughly
  1.18 million characters, still above the safe Codex transport limit.
- The remaining size is mainly exact source text, both compared answer sets, and the
  complete compact omission index. Removing enough of those inputs would weaken citation
  or omission review. Splitting and merging the review would add the orchestration
  complexity this development path is intended to avoid.
- The unused one-call implementation was removed. Retain the straightforward sequence of
  ten independent question reviews. The next optimization should target representation
  earlier in research/reading, where output can be reduced without withholding evidence
  from the quality reviewer.

## Shared evidence-table experiment rejected — 2026-08-14

- Measured a lossless, deterministic representation that stores repeated source passages
  once and lets evidence records reference them by hash. The frozen Netanyahu ledger
  contains 786 evidence records but 688 distinct passages; this reduced canonical JSON
  from 8.52M to 7.92M characters. Sharing repeated source URL, title, publisher, and
  document-hash metadata as well reduced it only to 7.78M characters, an 8.6% saving.
- Byte-equivalent reconstruction passed, but the saving is too small to justify another
  production storage contract. The prototype was removed and no model call was made.
- Most ledger volume is genuinely distinct exact excerpt text. The current reader already
  returns concise findings and source-unit ranges, and deterministic code already copies
  and hashes exact excerpts from frozen source text. The actual next cost slice is finer
  immutable citation granularity: measure extraction-unit and multi-unit span inflation,
  then test sentence/paragraph sub-units or validated character offsets that code can
  resolve byte-exactly. In the frozen Netanyahu ledger, exact excerpts alone occupy about
  7.03M characters; the median is 6,755 characters, 517 of 786 exceed 5,000 characters,
  and the largest is 142,455. No model call is warranted until a deterministic span
  prototype shows material savings without weakening provenance or corpus coverage.

## Paragraph citation-span prototype — 2026-08-14

- Added a deterministic, no-model prototype that addresses exact paragraph substrings by
  immutable extraction-unit number, start/end character offsets, and SHA-256. Resolution
  returns the original substring without normalization and rejects changed source text or
  invalid offsets.
- Across the frozen Netanyahu acquisition, 320 documents contain 8,600 extraction units
  and 20.38M source characters. Deterministic blank-line paragraph boundaries produce
  31,543 addressable spans covering 19.74M nonblank characters. Oversized segments above
  5,000 characters fall from 919 whole units to 480 paragraphs. Paragraph granularity is
  therefore materially better but does not eliminate every oversized passage.
- This remains an isolated representation prototype; existing evidence artifacts and the
  production reader/binder are unchanged. Next gate: independently review the offset and
  hashing contract, then test a small frozen source slice in which a reader selects
  labeled paragraph spans and code reconstructs exact quotations. No full-corpus or model
  rollout is authorized until that bounded comparison preserves evidence yield, locator
  fidelity, contrary evidence, and source coverage.

## Bounded paragraph-span reader comparison — 2026-08-14

- Ran one frozen Labour Force Survey slice (units 236–247) through Terra using paragraph
  labels, then compared it with the existing whole-unit evidence using Sol. All calls used
  the Codex subscription and no API key. The first attempt reduced cited text by 53% but
  omitted housing-tenure evidence. Supplying the frozen baseline fact as a coverage
  checkpoint restored the facts but omitted one interpretive limitation. Supplying the
  complete baseline fact plus its limitations produced three atomic findings with 81,331
  cited characters versus 142,455 in the baseline, a 42.9% reduction.
- The final independent labeled comparison considered the paragraph result at least
  equal: material
  coverage, balance, routing, attribution, and locator fidelity were preserved or improved,
  with no material omission. It also found that both baseline and experiment mislabel
  71.9% as an owner-occupied-dwelling employment rate; the table associates 71.9% with
  rented dwellings overall, while owner-occupied is 60.1%. The result remains diagnostic
  and is blocked from production. The comparison's model-authored relative conclusion is
  diagnostic rather than an eligibility gate. Promotion requires correcting that factual
  issue, deterministic pass/fail dimensions, exact trusted reconstruction of both stages,
  extraction and question-contract validation, transport preflight, and tamper tests.
- Completed a fresh corrected v4 diagnostic without altering the earlier artifacts. The
  correction record is bound to `BATCH-0021-R01-E006` and its source, changing the
  dwelling labels to 60.1% owner-occupied and 71.9% rented overall. Terra then returned
  two facts with 53,831 cited characters, a 62.2% reduction from the 142,455-character
  baseline. A separate Sol comparison passed factual accuracy, material coverage,
  balance, routing/attribution, and locator fidelity with no material omissions.
- Both stages now reconstruct exactly from the frozen extraction, corrected baseline,
  prompt and profile configurations, saved prompt/output/schema, and exact span offsets.
  The pass/fail result is derived by code rather than supplied by the model. This remains
  a bounded diagnostic: the result supports a later production design decision but does
  not modify the established reader/binder or add a review loop to the main pipeline.

## Paragraph citation production boundary — 2026-08-14

- Extended the existing reader fact intent with optional paragraph IDs and the existing
  binder with separately hash-bound exact citations. The binder resolves every ID against
  the frozen extraction and rejects unknown or duplicate IDs. Existing whole-unit
  excerpts remain unchanged, so historical artifacts and current consumers continue to
  load without migration.
- Multiple non-contiguous paragraphs remain separate citation objects; they are not
  concatenated into a synthetic quotation. This is the first production-boundary step,
  not a new pipeline phase and not yet a storage saving, because the legacy whole-unit
  excerpt remains present during compatibility migration.
- Next step: move the corpus-reader prompt to its versioned configuration and render
  stable paragraph labels in the existing reading call. Persist the prompt-config binding
  in the batch request, then run a deterministic prompt/binder integration test before
  considering any fresh model call or downstream consumer migration.
- Completed that prompt/binder wiring without a model call. The existing corpus-reading
  call now renders stable paragraph labels from `configs/corpus-reader-prompts.yaml`,
  records the prompt version/hash and exact request size, and returns selected IDs through
  the existing fact schema. A mocked end-to-end run proves the current binder resolves
  those IDs into exact citations without adding a phase.
- Added a configured 800,000-character content budget to the existing deterministic batch
  planner alongside its token budget. Rebuilding all five frozen ruler plans kept their
  largest complete serialized prompts between 824,075 and 825,847 characters, below the
  943,718-character safety ceiling. The earlier plans contained five oversized batches;
  no unchanged oversized request was launched or retried.
- Next gate: independently review the prompt/config, planner, request binding, and
  compatibility path. If clean, measure how many existing downstream consumers can read
  the structured citations directly before deciding whether a bounded fresh reader call
  is warranted.
- The integration gate passed. The immediate existing verifier now transports the
  separately hash-bound paragraph citations when they exist and omits the redundant
  whole-unit excerpt from that call only. Historical records without citations retain
  the whole-unit verifier payload. Stored evidence remains unchanged, and no verifier
  phase or call was added.
- Verification partitioning measures the actual selected transport record and fails
  before execution if one record cannot fit its configured bound. The next decision is a
  single bounded production-path reader/verification comparison on the same frozen slice;
  no broader corpus run is authorized yet.
- Ran that bounded production-path test with Terra through the Codex subscription (no API
  key). The reader returned three valid narrow facts, but omitted both principal known
  comparisons from the source slice. Verification corrected one overstatement but cannot
  recover facts that discovery omitted. The paragraph-labeled discovery prompt therefore
  failed the quality gate and is not promoted.
- Reverted active discovery to the proven whole-unit representation while retaining the
  versioned prompt/request binding, shared extraction validation, dual character/token
  planning, optional citation schema, and exact binder. The next design uses the existing
  verifier—not the discovery call—to select paragraph citations for already discovered
  facts. This preserves discovery quality and adds no model call or pipeline phase.
- Implemented that verifier-side design. Whole-unit discovery is rejected if it attempts
  to preselect paragraph IDs. The existing verifier receives every exact paragraph in the
  already-bound fact range, returns the supporting IDs with its normal verdict, and code
  attaches those separately hashed citations. Verifier requests bind the exact candidates,
  partitions, prompts, schema, profile, and model; current artifacts cannot be recovered
  through the explicitly marked pre-v2 legacy path.
- Removed the prior automatic verifier rerun. An empty, unknown, duplicate, or otherwise
  inconsistent citation selection now fails the batch once and requires an explicit later
  decision; the pipeline does not silently spend a second call.
- Ran one fresh bounded production comparison with that design, using exactly one Terra
  discovery call and one Terra verification call through the Codex subscription (no API
  key and no retry). The transport and trusted recovery path completed successfully. The
  reader preserved the material Jewish-Arab employment comparison, and the verifier bound
  four exact citation spans across three accepted facts.
- The result does not pass the promotion decision. It omitted the known dwelling-tenure
  comparison (60.1% employment for owner-occupied dwellings versus 71.9% for rented
  dwellings), and selected citation text fell only from 63,521 to 52,723 characters, a
  17.0% reduction. The source tables are commonly extracted as one large paragraph per
  page, so paragraph selection is too coarse to provide the expected saving. Keep the
  production reader on its existing whole-unit evidence contract. Do not add an automatic
  correction/review loop; any later optimization should address deterministic table-row
  granularity before another model comparison.
