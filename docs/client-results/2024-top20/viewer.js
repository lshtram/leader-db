(function () {
  ...
  const state = { data: null, rows: [], sortByAverage: false, selected: null };
  const head = document.getElementById("table-head");
  const body = document.getElementById("table-body");
  const record = document.getElementById("record");
  const filter = document.getElementById("filter");
  const sortButton = document.getElementById("sort-average");
  ...
  function renderHeader() {
    head.innerHTML = `<th scope="col">Ruler</th>${Object.entries(state.data.chapter_titles).map(([id, title]) => `<th scope="col" title="${esc(title)}">${esc(id)}<br>${esc(title.split(" ")[0])}</th>`).join("")}<th scope="col">Average</th>`;
  }

  function renderRows() {
    const query = filter.value.trim().toLowerCase();
    let rows = state.data.rulers.filter(row => `${row.ruler_name} ${row.country_name} ${row.iso3}`.toLowerCase().includes(query));
    if (state.sortByAverage) rows = [...rows].sort((a, b) => b.average_score - a.average_score);
    state.rows = rows;
    body.innerHTML = rows.map(row => `<tr data-iso3="${esc(row.iso3)}">
      ...
      ${Object.keys(state.data.chapter_titles).map(chapter => `<td><button class="score-button" type="button" data-iso3="${esc(row.iso3)}" data-chapter="${esc(chapter)}" aria-label="Open ${esc(row.ruler_name)} ${esc(chapter)} record">${score(row.automated_scores[chapter], "auto")}</button></td>`).join("")}
      <td class="mse">${row.average_score.toFixed(3)}<small>${row.scored_chapters}/8 scored</small></td>
    </tr>`).join("");
  }
  ...
  function chapterHtml(row, chapter, selectedChapter) {
    ...
    return `<details class="chapter" data-chapter="${esc(chapter.chapter_id)}"${chapter.chapter_id === selectedChapter ? " open" : ""}><summary><span class="chapter-code">${esc(chapter.chapter_id)}</span><span><span class="chapter-title">${esc(chapter.title)}</span><span class="chapter-scoreline">Score ${fmt(auto)}${chapter.recovery ? " · pending review" : ""}</span></span></summary><div class="chapter-body">${chapter.lenses.map(lens => lensHtml(lens, evidenceById)).join("")}${judgeHtml(chapter.judge, chapter.recovery)}</div></details>`;
  }
  ...
  function renderRecord(iso3, selectedChapter) {
    ...
    record.innerHTML = `<header class="record-header"><div><p class="eyebrow">Complete ruler record · ${esc(row.source_run)}</p><h2>${esc(row.ruler_name)}</h2><p>${esc(row.country_name)} · ${esc(row.iso3)} · ${state.data.target_year} · dossier ${esc(row.ruler_year_id)}</p></div><div class="record-mse"><span>Eight-chapter average</span><strong>${row.average_score.toFixed(3)}</strong><span>${row.scored_chapters} scored chapters</span></div></header><div class="chapter-list">${row.chapters.map(chapter => chapterHtml(row, chapter, selectedChapter)).join("")}</div>`;
  ...
  sortButton.addEventListener("click", () => {
    state.sortByAverage = !state.sortByAverage;
    sortButton.textContent = state.sortByAverage ? "Restore cohort order" : "Sort by average";
    renderRows();
  });
  ...
  fetch("data.json").then(response => {
    ...
    document.getElementById("cell-count").textContent = data.overall_scored_cells;
    document.getElementById("overall-average").textContent = data.overall_average.toFixed(3);
    ...
    record.innerHTML = `<div class="empty-state"><h2>Unable to load viewer data</h2><p>${esc(error)}</p><p>Serve this page through <code>python scripts/serve_design_reviews.py --page docs/client-results/2024-top20/index.html</code>.</p></div>`;
  });
})();
