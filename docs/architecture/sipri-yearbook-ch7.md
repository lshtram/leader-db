...
1. **`nuclear`** — 3 indicators: `sipri_yearbook_ch7_nuclear_warheads_total_inventory`, `sipri_yearbook_ch7_nuclear_warheads_deployed`, `sipri_yearbook_ch7_nuclear_warheads_retired`. All three feed the `nuclear` category. SIPRI Yearbook Ch.7 is the current Yearbook snapshot source; FAS Nuclear Notebook is now also implemented as the complementary clean `nuclear_country_year` source for consolidated-status cross-checks.

> **Why `nuclear` only, no other category?** The 3 indicators all measure aspects of nuclear arsenals (total count, operational count, retirement pipeline) — all proxies for the same underlying signal ("how big is this state's nuclear arsenal?"). This historical design note originally predated the FAS clean migration; as of 2026-06-27, FAS is also implemented as a clean Stage 2 / unified-source `nuclear_country_year` evidence source, while SIPRI Yearbook Ch.7 remains the current Yearbook snapshot source.
...
SIPRI Yearbook Ch.7 has no Stage 1 interaction — the client matrix is the 2023 validation/test reference and is read separately, never counted as source evidence. SIPRI Yearbook Ch.7 and FAS Nuclear Notebook are now both implemented nuclear evidence sources under the unified source interface; SIPRI carries the current Yearbook snapshot and FAS carries the consolidated status-page snapshot. The Stage 2 → Stage 12 (compare-vs-client) flow is unchanged by either source's presence (the client matrix has its own `nuclear` column that adapters do not overwrite).
...
5. **No cross-validate-with-other-source output.** SIPRI milex is the 2nd source for the `international_peace` category (cross-validating UCDP). This SIPRI Yearbook Ch.7 Stage 2 adapter does not itself produce cross-source comparison output; as of the 2026-06-27 clean-source migration, FAS is also implemented as the complementary `nuclear_country_year` source, and any SIPRI/FAS agreement or temporal-fit handling belongs in downstream scoring/confidence stages, not in this adapter.
