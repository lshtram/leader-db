...
`leaders_db.sources` migrations one source at a time. WHO GHO API, FAS, and
Wikidata WikiProject heads-of-state-and-government are now migrated under
...
API, FAS, and Wikidata HoS/HoG, the unified source interface now covers
...
health country-year evidence, and Wikidata per-binding leader-identity
evidence. **Active next action:** project-manager review + reviewer pass for
the Wikidata HoS/HoG clean migration, then choose the next clean source
migration or resume the vertical-slice investigation through the migrated
source pipeline. The next pending legacy-implemented clean-source row in
the inventory is `wikipedia_search_extract` unless the project-manager chooses
