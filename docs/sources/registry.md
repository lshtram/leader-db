...
| `polity_v` | ✅ | Polity V dataset | Direct `.sav` file from inscrdata.html; 1800–2018, 167 countries. Runtime metadata/checksum repaired and the clean adapter persisted 193,028 validated observations (11 indicators, 1800–2018) on 2026-07-22. It is a historical backstop only: 2022 requests return an explicit `year_absent` warning and no proxy rows. **Fallback to Freedom House/V-Dem/EIU for 2022+.** |
...
| `un_snaama` | ✅ | UNSD National Accounts Main Aggregates Database | Local UNdata export dated 2026-06-19; 1970–2024; current-price US-dollar GDP and five expenditure aggregates. Fully normalized, persisted, concept-mapped, country-matched, and routed to relevant 5B lenses with nominal-not-real-growth warnings. |
