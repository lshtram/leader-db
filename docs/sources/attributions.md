...
### `ctbto_treaty_status` — CTBTO States Signatories (CTBT signature and ratification status)

- **What we extract:** the canonical CTBTO States Signatories page (`https://www.ctbto.org/our-mission/states-signatories`) — a single-point treaty-status snapshot covering ~196 States, with columns `Region` / `State` / `Signature Date` / `Ratification Date`. The clean adapter emits one observation per cached State row + source-derived status indicator (2 source-derived catalog indicators per row by default: `ctbto_treaty_status_signature_status` (`"signed"` iff the signature date is non-empty, `"not_signed"` otherwise) and `ctbto_treaty_status_ratification_status` (`"ratified"` iff the ratification date is non-empty, `"not_ratified"` otherwise)). The transform layer never invents a signature / ratification status from empty / blank date cells -- unsigned / unratified rows are NOT silently treated as signed / ratified. The catalog deliberately does NOT include a default `ctbto_treaty_status_annex_2_status` indicator -- the canonical CTBTO public page does NOT carry an Annex 2 flag column; when a cached fixture / source-native data carries an explicit `Annex 2` column the transform layer emits an OPTIONAL per-row Annex 2 observation, but the default 2-indicator catalog does NOT include the Annex 2 indicator so the adapter never invents an Annex 2 flag from missing source-native data. Raw signature / ratification dates are preserved verbatim as strings on the audit-trail extension payload (the adapter does NOT coerce them to numeric years that could mislead Stage 11 confidence calculations). The source is offline / cache-only in this slice; it reads a staged cached CSV (or HTML fallback) plus a runtime-local `metadata.json` (gitignored per Always-On Rule #9) from `data/raw/ctbto_treaty_status/`. Live fetch is intentionally NOT supported — `cache_policy='refresh'` / `'no_cache'` fails readiness with a structured `ctbto_treaty_status_unsupported_cache_policy` error.
- **What we don't use:** per-State inferences about nuclear behaviour, compliance, or non-compliance are NOT made by the adapter; the CTBT signature / ratification status is a treaty-status observation, NOT direct proof of nuclear behaviour, compliance, or non-compliance. The adapter also does NOT infer ISO3 country codes (the CTBTO States Signatories page uses CTBTO's own State display names, which are not ISO3); `country_code` remains `None` until later matching / resolution stages introduce a canonical ISO3 mapping. The pipeline does NOT redistribute the copied full CTBTO table in public outputs per the CTBTO terms-of-use; the clean adapter carries the `source_url` and the per-row source-native State / Region display labels + date cells on the audit-trail extension payload so downstream code can recover the verbatim source.
- **Important caveat:** this source captures treaty-status evidence — the presence / status of a State signature and ratification of the Comprehensive Nuclear-Test-Ban Treaty — and is NOT direct proof of nuclear behaviour, compliance, or non-compliance by itself. Downstream scorers MUST NOT silently treat an unsigned / unratified status cell as proof of nuclear activity or non-cooperation; the descriptor's `coverage_hint.notes` carries the explicit caveat and the Stage 11 confidence formula penalises the temporal-fit gap between the cached snapshot date (2024-03-13) and the prototype's target year (2023). The CTBTO States Signatories table is a single-point treaty-status snapshot; the descriptor advertises a single-year 2024 envelope so out-of-coverage year requests (e.g. `years=(2023,)`) emit zero observations plus a structured `YEAR_ABSENT` warning (no stale-proxy fill).
- **License:** CTBTO terms-of-use (`https://www.ctbto.org/terms-of-use`) permit users to visit, download and copy CTBTO materials subject to terms; personal, non-commercial, research / teaching use is permitted with acknowledgement; no resale, redistribution, or derivative compilation without permission; cite the CTBTO + the canonical URL.
- **Citation:**
  > Comprehensive Nuclear-Test-Ban Treaty Organization. 2024. *States Signatories — Comprehensive Nuclear-Test-Ban Treaty* (status as of 13 March 2024). Vienna: CTBTO. https://www.ctbto.org/our-mission/states-signatories
- **Attribution text in reports:** "CTBTO States Signatories, Comprehensive Nuclear-Test-Ban Treaty signature and ratification status (Comprehensive Nuclear-Test-Ban Treaty Organization, status as of 13 March 2024)."

### `world_bank_poverty_inequality_platform` — World Bank Poverty and Inequality Platform (PIP)

- **What we extract:** country-year poverty / inequality / distribution observations from the World Bank PIP — the documented per-(country_code, country_name, year, reporting_level, welfare_type, poverty_line) indicators such as the poverty headcount ratio, the poverty gap, and the Gini index. The clean adapter emits one observation per cached row + source-native catalog indicator (3 per-row indicators: `world_bank_poverty_inequality_platform_poverty_headcount_ratio` / `world_bank_poverty_inequality_platform_poverty_gap` / `world_bank_poverty_inequality_platform_gini_index`). The transform never invents a value from missing source-native data; blank / non-numeric cells are emitted as `value=None` / `value_type="missing"` plus the verbatim raw cell text on `extension.raw_value`. The source-native `country_code` (the World Bank's own reporting identifier) is preserved verbatim on the audit-trail extension payload; the adapter does NOT assume the source identifier is a canonical ISO3 even when it resembles one. Per-row PPP version + reporting level + welfare type + poverty line + version_id are preserved on the audit-trail extension payload so downstream code can recover the verbatim source-native provenance.
- **What we don't use:** live HTTP fetch is intentionally NOT supported in this slice — `cache_policy='refresh'` / `'no_cache'` fails readiness with a structured `world_bank_poverty_inequality_platform_unsupported_cache_policy` error. Per-country inferences about poverty / inequality are NOT made by the adapter; PIP captures survey / model / PPP-specific observations, NOT direct poverty / inequality rankings. The PIP estimates are SURVEY- and PPP-specific and SHOULD NOT be silently mixed across PIP version stamps (e.g. `20260324_2021` vs `20260324_2017`) or PPP bases (2021 PPP vs 2017 PPP) without explicit metadata propagation. The adapter propagates the canonical PIP version stamp + PPP version (when present) onto every emitted observation's audit-trail extension payload so downstream scorers can apply the canonical "version / PPP basis" treatment explicitly.
- **Important caveat:** PIP poverty / inequality estimates are SURVEY- and PPP-specific and SHOULD NOT be silently mixed across PIP version stamps or PPP bases without explicit metadata propagation. The descriptor's `coverage_hint.notes` carries the explicit caveat and the Stage 11 confidence formula penalises the temporal-fit gap between the cached PIP version release date and the prototype's target year. The canonical version stamp `"World Bank PIP, version 20260324_2021"` propagates consistently to `RawAsset.version` and every emitted `NormalizedObservation.source_version`. The descriptor advertises a broad 1960-2024 coverage envelope so out-of-coverage year requests (e.g. `years=(2050,)`) emit zero observations plus a structured `YEAR_ABSENT` warning (no stale-proxy fill); the prototype's target year 2023 falls WITHIN the canonical envelope so 2023 is in-coverage.
- **License:** World Bank Terms of Use for Datasets; free use with attribution; cite the World Bank Group + the canonical PIP URL.
- **Citation:**
  > World Bank. 2025. *Poverty and Inequality Platform (version 20260324_2021) [Data set]*. Washington, D.C.: World Bank Group. https://pip.worldbank.org/ Licensed under the World Bank Terms of Use for Datasets.
- **Attribution text in reports:** "World Bank (2025) Poverty and Inequality Platform (version {version_ID}) [Data set] World Bank Group, www.pip.worldbank.org."
...
| `ctbto_treaty_status` | CTBT treaty-status observations (CTBT signature / ratification status per State) | single-point snapshot (2024-03-13) | CTBTO terms-of-use; personal, non-commercial, research / teaching use with acknowledgement; no redistribution of copied full table | "CTBTO States Signatories, Comprehensive Nuclear-Test-Ban Treaty signature and ratification status (Comprehensive Nuclear-Test-Ban Treaty Organization, status as of 13 March 2024)." |
| `world_bank_poverty_inequality_platform` | poverty / inequality / distribution observations (headcount / poverty gap / Gini) | 1960–2024 (canonical PIP version 20260324_2021) | World Bank Terms of Use for Datasets; free use with attribution | "World Bank (2025) Poverty and Inequality Platform (version {version_ID}) [Data set] World Bank Group, www.pip.worldbank.org." |
