...
| **I10** | Partial. | Generic cited/manual persistence exists; remaining work is eight chapter-guide smoke batches, chapter-score persistence wiring, and calibrated eight-judge execution. |
...
| **D25** | `ruler_year_question_answers` | One answer per ruler/country/year/question for 1B-8B. | 1B-8B. | I4, I7, I8, I10 | Uses the same generic `research_question_answers` path for now, keyed by question/year/ISO3/method and optional ruler fields. The target acquisition topology is one long-running evidence researcher per resolved ruler-period, not one researcher per ruler-question. Each researcher writes one reusable cited dossier with stable evidence IDs and many-to-many mappings from evidence to every applicable 1B-8B question. After evidence collection, one question-specific judge consumes that question's mapped evidence across all eligible rulers for the year and writes the D25 answers using one versioned rubric/calibration meter. `persist_cited_evaluations` now persists already-cited registered `internet_manual` outputs for non-8B and 8B methodology questions through this path; `leaders-db research persist-cited-evaluations --input <file.json>` is the generic CLI entrypoint. `leaders-db research cited-evaluation-schema` and `leaders-db research cited-evaluation-template` define the current evaluation form, which will be separated into dossier/mapping and judge-output contracts as this topology lands. Score-bearing records require `answer_payload.calibration` per `docs/methodology/cited-evaluation-calibration.md`; each question needs a guide under `docs/methodology/question-guides/` before judge smoke tests. The older `persist_effectiveness_8b_evaluations` / `persist-8b-evaluations` path remains as an 8B-specific compatibility wrapper. Dedicated ruler-year tables are deferred until answer shapes stabilize. |
...
5. **I10 manual/internet evaluator storage:** generic cited-evaluation persistence
   exists, but the active target is eight chapter-level smoke slices. Run ruler
   dossiers, have one `ruler-chapter-judge` synthesize each chapter's ten lenses,
   and persist one cited chapter score plus confidence per ruler.
...
5. **Populate D25/D26:** generic cited/manual persistence now exists via
    ...
    chapter-calibration metadata, and each chapter uses one guide under
    `docs/methodology/chapter-guides/` before its evidence/judge smoke test.
    Remaining D25/D26 work is chapter-score persistence and the ruler-period
