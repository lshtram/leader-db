"""Render a blinded evaluator prompt for the ten-case chapter A/D experiment."""
...
def main() -> None:
    """Print all comparisons with labels in the requested order."""

    parser = argparse.ArgumentParser()
    parser.add_argument("experiment_dir", type=Path)
    parser.add_argument("--reverse", action="store_true")
    args = parser.parse_args()
    first, second = ("d", "a") if args.reverse else ("a", "d")
    manifest = json.loads(
...
Finish with an overall recommendation. Explain whether the ten-case evidence is strong
enough to promote one prompt to the next end-to-end gate. Return Markdown.
"""
    )
